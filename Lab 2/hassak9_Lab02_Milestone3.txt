...
Locked_State_BIN    EQU 2_00
; No input => 000, locked state 1 => 010, locked state 2 => 100, locked state 3 => 110, unlocked state => 001
...
;Function PortF_Init. LED3 (PF4) 
PortF_Init 
    ;STEP 1
    LDR R1, =SYSCTL_RCGCGPIO_R 
    LDR R0, [R1]   
    ORR R0,R0, #0x20      ; Activating Port F       				          
    STR R0, [R1]               
    NOP 
    NOP   
    ...
    LDR R0, [R1] 
    ORR R0,R0, #2_00010000      ; Activating pin 4 as an output    
	STR R0, [R1]   
    
    ;STEP 7
    LDR R1, =GPIO_PORTF_DEN_R   
    LDR R0, [R1] 
    ORR R0, R0, #2_00010000    ; Enabling pin 4 as digital output                                
    STR R0, [R1]  
    BX  LR                            
...
; Function PortM_Init. used for buttons (PM0) input and (PM4) Logic testing
PortM_Init 
    ;STEP 1
	LDR R1, =SYSCTL_RCGCGPIO_R       
	LDR R0, [R1]   
...
State_Init LDR R5,=GPIO_PORTN_DATA_R  ; Locked is the initial state, no LEDs ON
		   MOV R4,#2_00000000         
	       STR R4,[R5]
	       BX LR 
...
Start 
    BL  PortF_Init
	BL  PortN_Init                    ;Function calls to initialize ports and pins
	BL  PortM_Init
	;BL  State_Init
	LDR R0, =GPIO_PORTM_DATA_R        ; Setting pointers to relevant input and output port data registers 
	LDR R4, =GPIO_PORTN_DATA_R         
	
	LDR R7, =COUNT_REMAINING          ; Storing values of countrol variables count remaining and locked state binary         
	LDR R8, =Locked_State_BIN		  ; value using registers and pointers
...
CorrectEntry
			...
			BEQ Unlocked_State       
			ADD R8,#2_01
			STR R8, [R4]		; if not, we have a correct entry, just not enough, so we subtract 1 from the count remaining variable, add 1 to the locked state variable 
			SUB R7,#1           ; and check the next entry
			
			B IsZero                  
 
Locked_State         
	LDR R5,=GPIO_PORTF_DATA_R
	MOV R6,#2_00000000         ; LED3 OFF
	STR R6,[R5]
	
	LDR R7, = 3           ; We have incorrect inputs => reset control variables and check for new inputs (4 needed)           
	LDR R8, = 0
	STR R8, [R4]
	B IsZero
	
Unlocked_State; LED3 is ON for unlocked state
	LDR R5, =GPIO_PORTF_DATA_R
	MOV R6,#2_00010000             ; LED3 ON
	STR R6, [R5]
	
	LDR R7, = 3		; correct sequence => unlock and reset variables to check next 4 inputs
	LDR R8, = 0
	STR R8, [R4]

	B Start ; and start over
